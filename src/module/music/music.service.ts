...
export class MusicService {
  ...
  async createMusic(
    ...
    const lastMusic = await this._prismaService.music.findFirst({
      where: { userId },
      orderBy: { createdAt: 'desc' },
    });

    if (lastMusic) {
      const oneHourLater = addHours(lastMusic.createdAt, 1);
      if (
        isAfter(new Date(), lastMusic.createdAt) &&
        new Date() < oneHourLater
      ) {
        throw new BadRequestException(
          'Anda hanya bisa generate music setiap 1 jam sekali',
        );
      }
    }
